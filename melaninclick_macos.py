...
class Application(tk.Tk):
    ...
    def check_os_compatibility(self):
        if platform.system() != "Darwin":
            messagebox.showerror("OS Error", "This app is designed for macOS only.")
            self.exit_app()
...
class InstallPage(ttk.Frame):
    ...
    def check_storage_and_install_whive(self):
        ...
        if free_space > 10:
            threading.Thread(
                target=self.install,
                args=('whive', "https://github.com/whiveio/whive_releases/releases/download/22.2.3/whive-ventura-22.2.3-osx64.tar.gz", False)
            ).start()
        else:
            self.update_output(f"Insufficient space: {free_space:.2f} GB available.", "error")
            self.progress.stop()
