...
use std::fs;
use tauri::State;
use crate::{AppState, AppError, NodeStatus};
use crate::core::{get_process_manager, ensure_directory_exists};
...
pub async fn download_and_install_bitcoin(state: State<'_, AppState>) -> Result<String, AppError> {
    ...
    Ok("Bitcoin Core installed successfully. Both bitcoind and bitcoin-qt are available.".to_string())
}
...
pub async fn download_and_install_whive(state: State<'_, AppState>) -> Result<String, AppError> {
    ...
    Ok("Whive Core installed successfully. Includes whived, whive-qt, and minerd for mining.".to_string())
}
...
pub async fn run_bitcoin_mainnet(use_qt: Option<bool>) -> Result<String, AppError> {
    ...
    Ok(format!("Started Bitcoin mainnet node ({}) with PID: {}", executable_name, pid))
}
...
pub async fn run_bitcoin_pruned(use_qt: Option<bool>) -> Result<String, AppError> {
    ...
    let executable_name = if prefer_qt { "bitcoin-qt" } else { "bitcoind" };
    Ok(format!("Started Bitcoin pruned node ({}) with PID: {}", executable_name, pid))
}
...
pub async fn run_whive_node(use_qt: Option<bool>) -> Result<String, AppError> {
    ...
    Ok(format!("Started Whive node ({}) with PID: {}", executable_name, pid))
}
...
pub async fn get_node_status(node_type: String) -> Result<NodeStatus, AppError> {
    ...
    Ok(NodeStatus {
        ...
        data_dir: format!("~/.{}", if node_type.contains("bitcoin") { "bitcoin" } else { "whive" }),
        config_path: format!("~/.{}/{}.conf", 
                           if node_type.contains("bitcoin") { "bitcoin" } else { "whive" },
                           if node_type.contains("bitcoin") { "bitcoin" } else { "whive" }),
    })
}
...
async fn get_bitcoin_download_url() -> Result<String, AppError> {
    ...
    let url = match (os_type, arch) {
        ...
        ("windows", _) => format!("https://bitcoincore.org/bin/bitcoin-core-{}/bitcoin-{}-win64.zip", version, version),
...
async fn download_file_internal(
    ...
    if !response.status().is_success() {
        return Err(AppError::Node(format!("Failed to download: HTTP {}", response.status())));
    }
...
async fn create_bitcoin_conf(conf_path: &Path, prune: bool) -> Result<(), AppError> {
    let mut conf_content = vec![
        ...
        "rpcpassword=melanin_secure_password_2024".to_string(),
...
fn find_executable_in_path_sync(base_path: &Path, executable_name: &str) -> Result<PathBuf, AppError> {
    ...
    search_recursive(base_path, executable_name)
        .ok_or_else(|| AppError::Node(format!(
